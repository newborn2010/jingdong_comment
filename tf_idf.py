...
def tf_idf(input_path, stop_word=None, show=10):
    ...
    input_path: list
    e.g.
    input_path = ['cut1.txt', 'cut2.txt', 'cut3.txt']
    ...
    print('Loading done :', datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    # 设置停用词
    if stop_word == None:
    ...
    print('Model done :', datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    # 结果
    result = []
    for i in range(array.shape[0]):
        ...
        for j in sort:
            topic.append(name[j])
        result.append(('corpus'+str(i+1), topic))
